...
namespace PerformanceCheck {
   ...
   class DictionaryPerformance {
      ...
      const int Second = 1000; // 1 Second = 1000 millisecond
      ...
      public void TraverseInt( ) {
         ...
         while (mTimer1.ElapsedMilliseconds <= Second) {
            for (int i = 0; i < IntMax; i++) {
               mDumb = mDictWithIntAndObject[mRand.Next(0,IntMax)];
            }
            mCount++;
         }
         mTimer1.Stop ();
         Console.WriteLine ("Retrieves ~" + (mCount * IntMax) / 1000000 + " million (random) records in " + convertMilliSecondToSecond (mTimer1.ElapsedMilliseconds) + " seconds from " + IntMax + " records.");
      }
      ...
      public void TraverseShort( ) {
         ...
         while (mTimer1.ElapsedMilliseconds <= Second) {
            for (short i = 0; i < ShortMax; i++) {
               mDumb = mDictWithShortAndObject[(short) mRand.Next (0, ShortMax)];
            }
            mCount++;
         }
         mTimer1.Stop ();
         Console.WriteLine ("Retrieves ~" + (mCount * ShortMax) / 1000000 + " million (random) records in " + convertMilliSecondToSecond (mTimer1.ElapsedMilliseconds) + " seconds from " + ShortMax + " records.");
      }
      ...
      public void TraverseUShort( ) {
         ...
         while (mTimer1.ElapsedMilliseconds <= Second) {
            for (ushort i = 0; i < UShortMax; i++) {
               mDumb = mDictWithUShortAndObject[(ushort) mRand.Next (0, UShortMax)];
            }
            mCount++;
         }
         mTimer1.Stop ();
         Console.WriteLine ("Retrieves ~" + (mCount * UShortMax) / 1000000 + " million (random) records in " + convertMilliSecondToSecond (mTimer1.ElapsedMilliseconds) + " seconds from " + UShortMax + " records.");
      }
